 ...
 */
public class StartCassandraClusterMojo extends AbstractCassandraMojo
{
    /**
    ...
    public void execute() throws MojoExecutionException, MojoFailureException
    {
        ...
        for (int node = 0; node < clusterSize; node++) {
            ...
            initialToken[node] = BigInteger.valueOf(2).pow(127).multiply(BigInteger.valueOf(node)).divide(BigInteger.valueOf(clusterSize));
        ...
        try
        {
            for (int node = 0; node < clusterSize; node++) {
                ...
                Utils.startCassandraServer(cassandraDir[node],
                        newServiceCommandLine(cassandraDir[node], listenAddress[node], listenAddress[node],
